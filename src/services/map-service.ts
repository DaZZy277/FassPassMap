...
export type UniversityMode = 'KMITL' | 'KMUTT'; // Type for university switching
...
export class MapService {
    ...
    public universityMode: UniversityMode = 'KMITL'; 
    ...
    // CORRECTED ZONES HASHES (Using central campus hashes)
    ...
    // KMUTT targets will be dynamically loaded/set based on switch
    private kmitlTargets: TargetLocation[] = [
    ...
    private kmuttTargets: TargetLocation[] = [
    ...
    constructor(@Inject(PLATFORM_ID) private platformId: Object) {
        this.targetsSubject.next(this.kmitlTargets); // Start with KMITL
        this.setupSearchStream();
    }
    
    // NEW: Logic to switch between universities
    public switchUniversity(mode: UniversityMode, L: any): void {
        if (this.universityMode === mode) return;

        this.universityMode = mode;
        
        const newTargets = mode === 'KMITL' ? this.kmitlTargets : this.kmuttTargets;
        this.targetsSubject.next(newTargets);
        
        // Recalculate distance and ranking based on new targets
        this.updateDistancesAndRanks(L); 
    }
    // --- ICON UTILITIES (Requires L as argument) ---
    ...
    public startGeolocationTracking(L: any) {
        if (!isPlatformBrowser(this.platformId) || !navigator.geolocation) {
            console.error("Geolocation not supported or running on server.");
            return;
        }
    ...
    public stopTracking() {
        ...
        this.userLat = null;
        this.userLng = null;
        this.targetsSubject.next(this.targetsSubject.getValue()); // Trigger update to clear distance/ranks
    }

    // NEW: Function to force the user's position manually
    public setManualPosition(lat: number, lng: number, L: any): void {
        this.userLat = lat;
        this.userLng = lng;

        if (this.isNgeohashInitialized()) {
            // Encode GeoHash for visualization
            this.userGeoHash = this.ngeohash.encode(lat, lng, 8);
        }
        
        // Trigger the distance calculation immediately
        this.updateDistancesAndRanks(L);
    }
    ...
    private setupSearchStream() {
        this.searchSubscription = this.searchQuerySubject.pipe(
            ...
            switchMap(query => {
                ...
                // The map operator handles setting isSearching back to false
    ...
    private _performSearch(query: string): Observable<SearchResult[]> {
        ...
        // Simulating asynchronous API call (0.5s latency)
    ...
    // --- PRIVATE LOGIC (Internal Data Processing) ---
